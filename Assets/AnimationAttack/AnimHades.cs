...
public class SwordRain : MonoBehaviour
{
    ...
    private float rainDuration = 5f; 

    private float timer = 0f;
    private float rainTimer = 0f;
    private bool isRaining = true; // Contrôle si la pluie est active
    ...
    System.Collections.IEnumerator FallSword(GameObject sword)
    {
        ...
        while (sword.transform.position.y > hexagonBase.transform.position.y)
        {
            sword.transform.Translate(Vector3.down * fallSpeed * Time.deltaTime);
            yield return null; 
        }
